class Serie {
  ...
  Serie() {
    ...
    t = width/cant;
    haySonido = false;
    paleta =  new color[] {color(#BDB351), color(#BBBBB3), color(#BFBE7D), color(#C9C1BC), color(#e1f2a7)};
    paleta_ = new color[] {color(#021859), color(#29524A), color(#545931), color(#2F524C), color(#283C54)};
    ...
    if (haySonido) {
      ...
      for (int i = 0; i < cant; i++) {
        for (int j = 0; j < cant; j++) {
          ...
          float p = map(amp, minAmp, maxAmp, 0, 2.5);
          float p1 = map(pitch, minPitch, maxPitch, 0, 1);
          ampAmortiguado = map(amp, minAmp, maxAmp, 0.1, 0.9);
          pitchAmortiguado = map(pitch, minPitch, maxPitch, 0.2, 0.99);
          if (p1 > 0 && p1 <0.7) {
